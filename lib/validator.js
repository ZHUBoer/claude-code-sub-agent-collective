...
class CollectiveValidator {
  ...
  async validateInstallation() {
    ...
    const fileChecks = [
      ...
      {
        name: "测试目录（.claude-collective/tests）",
        path: ".claude-collective/tests",
      },
  ...
  async validateSettings() {
    ...
    try {
      ...
      const hasPreToolUse =
        hasHooks && Array.isArray(settings.hooks.PreToolUse);
      const hasPostToolUse =
        hasHooks && Array.isArray(settings.hooks.PostToolUse);
      const hasSubagentStop =
        hasHooks && Array.isArray(settings.hooks.SubagentStop);
      const hasSessionStart =
        hasHooks && Array.isArray(settings.hooks.SessionStart);
      ...
      const bindingsOk =
        autoBranchBound && autoCheckpointBound && autoSquashBound;
      ...
      if (!bindingsOk) {
        return {
          ...
          passed: false,
          error:
            "缺少必需的 hook 绑定：请确保 SessionStart 含 auto-branch，PostToolUse 含 auto-checkpoint/auto-squash",
        };
      }
  ...
  async validateAgents() {
    ...
    try {
      ...
      const agentDefinitionFiles = agentFiles.filter(
        ...
        try {
          ...
          if (file.endsWith(".json")) {
            ...
            const hasDescription =
              agent.description && typeof agent.description === "string";
    ...
    try {
      ...
      if (!dependenciesInstalled) {
        return {
          ...
          passed: true,
          warning:
            "Test dependencies not installed (run npm install in .claude-collective/)",
          error: null,
        };
      }
