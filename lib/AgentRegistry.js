/**
 * claude tdd agents - Agent Registry
 *
...
class AgentRegistry extends EventEmitter {
  constructor(options = {}) {
    ...
    this.registryDir =
      options.registryDir || path.join(process.cwd(), ".claude", "state");
  ...
  async unregister(agentId, options = {}) {
    ...
    this.statistics.activeAgents = Math.max(
      0,
      this.statistics.activeAgents - 1
    );
  ...
  query(criteria = {}) {
    ...
    if (criteria.tools) {
      const requiredTools = Array.isArray(criteria.tools)
        ? criteria.tools
        : [criteria.tools];
      results = results.filter((agent) =>
        requiredTools.every(
          (tool) => agent.metadata.tools && agent.metadata.tools.includes(tool)
        )
      );
    }
    ...
    if (criteria.createdAfter) {
      ...
      results = results.filter(
        (agent) => new Date(agent.metadata.registeredAt) > cutoff
      );
    }
    ...
    if (criteria.healthStatus) {
      results = results.filter(
        (agent) => agent.health.status === criteria.healthStatus
      );
    }

    if (criteria.minSuccessRate !== undefined) {
      results = results.filter(
        (agent) => agent.performance.successRate >= criteria.minSuccessRate
      );
    }

    if (criteria.maxResponseTime !== undefined) {
      results = results.filter(
        (agent) => agent.performance.avgResponseTime <= criteria.maxResponseTime
      );
    }
  ...
  async updateActivity(agentId, activityData = {}) {
    ...
    if (activityData.success !== undefined) {
      ...
      // Update success rate
      const totalRequests =
        agent.activity.successCount + agent.activity.errorCount;
      agent.performance.successRate =
        agent.activity.successCount / totalRequests;
    }
    ...
    try {
      ...
      if (agent.path && (await fs.pathExists(agent.path))) {
        try {
          ...
          if (
            content.includes("corrupted content") ||
          ...
          if (
            ...
            healthCheck.issues.push(
              "Agent file does not contain valid markdown structure"
            );
            healthCheck.status = "unhealthy";
          }
        } catch (fileError) {
          healthCheck.issues.push(
            `Cannot read agent file: ${fileError.message}`
          );
          healthCheck.status = "unhealthy";
        }
      }
      ...
      if (agent.performance.successRate < 0.8) {
        healthCheck.issues.push(
          `Low success rate: ${(agent.performance.successRate * 100).toFixed(
            1
          )}%`
        );
        healthCheck.status = "warning";
      ...
      if (agent.performance.avgResponseTime > 30000) {
        // 30 seconds
        healthCheck.issues.push(
          `High response time: ${(
      ...
      if (agent.resources.memoryUsage > 100 * 1024 * 1024) {
        // 100MB
  ...
  getStatistics() {
    ...
    agents.forEach((agent) => {
      templateCounts[agent.template] =
        (templateCounts[agent.template] || 0) + 1;
    });

    // Health status distribution
    const healthCounts = {};
    agents.forEach((agent) => {
      healthCounts[agent.health.status] =
        (healthCounts[agent.health.status] || 0) + 1;
    });
    ...
    const avgResponseTime =
      agents.length > 0
        ? agents.reduce(
            (sum, agent) => sum + agent.performance.avgResponseTime,
            0
          ) / agents.length
        : 0;
  ...
  async cleanupAgentFiles(agent) {
    try {
      ...
      console.warn(
        `Failed to cleanup files for agent ${agent.id}:`,
        error.message
      );
    }
  }
  ...
  async createBackup() {
    try {
      ...
      const backupFile = path.join(
        this.backupDir,
        `registry-backup-${timestamp}.json`
      );
  ...
  async runHealthChecks() {
    ...
    for (const agentId of agents) {
      try {
        ...
        console.warn(
          `Health check failed for agent ${agentId}:`,
          error.message
        );
      }
    }
