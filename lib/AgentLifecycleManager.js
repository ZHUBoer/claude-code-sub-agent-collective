/**
 * claude tdd agents - Agent Lifecycle Manager
 *
...
class AgentLifecycleManager extends EventEmitter {
  ...
  async initialize() {
    ...
    this.registry.on("agent-activity-updated", (data) =>
      this.onAgentActivity(data)
    );
    this.registry.on("agent-health-checked", (result) =>
    ...
    try {
      ...
      for (const agent of agents) {
        try {
          ...
          console.warn(
            `Health check failed for agent ${agent.id}:`,
            error.message
          );
        }
      }
      ...
      this.emit("health-monitoring-completed", {
        ...
        healthyAgents: healthResults.filter((r) => r.status === "healthy")
          .length,
        unhealthyAgents: healthResults.filter((r) => r.status === "unhealthy")
          .length,
      });
    } catch (error) {
  ...
  async evaluateScaling() {
    ...
    try {
      ...
      if (
        loadMetrics.avgLoad > this.config.scaleUpThreshold &&
        activeCount < this.config.maxActiveAgents
      ) {
        scalingAction = "scale-up";
      } else if (
        loadMetrics.avgLoad < this.config.scaleDownThreshold &&
        activeCount > 1
      ) {
  ...
  async evaluatePolicies(agent) {
    ...
    for (const [policyId, policy] of this.policies) {
      ...
      try {
        ...
        console.warn(
          `Policy evaluation failed for ${policyId}:`,
          error.message
        );
      }
    }
  ...
  determineAction(violations) {
    ...
    const sorted = violations.sort(
      (a, b) => a.policy.priority - b.policy.priority
    );
  ...
  calculateLoadMetrics(agents) {
    ...
    const totalInvocations = agents.reduce(
      (sum, agent) => sum + agent.activity.invocations,
      0
    );
  ...
  addPolicy(policyId, policyConfig) {
    ...
    if (
      !policyConfig.condition ||
      typeof policyConfig.condition !== "function"
    ) {
