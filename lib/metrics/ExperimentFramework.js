...
class ExperimentFramework extends EventEmitter {
  constructor(metricsCollectors, options = {}) {
    ...
    this.options = {
      storageDir:
        options.storageDir ||
        path.join(process.cwd(), ".claude-collective", "experiments"),
      defaultSignificanceLevel: options.significanceLevel || 0.05,
      minSampleSize: options.minSampleSize || 30,
      maxExperimentDuration:
        options.maxExperimentDuration || 7 * 24 * 60 * 60 * 1000, // 7 days
  ...
  async createExperiment(config) {
    ...
    const experiment = {
      ...
      allocation:
        config.allocation || this.calculateEqualAllocation(config.variants),
      criteria: {
        successMetric: config.successMetric,
        minimumEffect: config.minimumEffect || 0.1,
        significanceLevel:
          config.significanceLevel || this.options.defaultSignificanceLevel,
  ...
  assignVariant(experimentId, subjectId, context = {}) {
    ...
    if (subjectAssignments[experimentId]) {
      return experiment.variants.find(
        (v) => v.id === subjectAssignments[experimentId]
      );
    }
    ...
    const analysis = {
      ...
      conversionRate:
        results.assignments > 0
          ? results.conversions.length / results.assignments
          : 0,
      metrics: {},
    };
  ...
  performStatisticalAnalysis(experiment, variants) {
    ...
    analysis.overallSignificance = this.determineOverallSignificance(
      analysis.correctedComparisons
    );

    // Recommend winner
    analysis.recommendedWinner = this.recommendWinner(
      experiment,
      variants,
      analysis
    );
  ...
  performSignificanceTest(
    ...
    const pooled =
      ...
      (controlTrials + treatmentTrials);

    const se = Math.sqrt(
      pooled * (1 - pooled) * (1 / controlTrials + 1 / treatmentTrials)
    );
  ...
  applyMultipleTestingCorrection(comparisons) {
    ...
    if (method === "bonferroni") {
      return comparisons.map((comp) => ({
        ...comp,
        correctedPValue: Math.min(
          comp.significance.pValue * comparisons.length,
          1
        ),
  ...
  determineOverallSignificance(correctedComparisons) {
    const significantComparisons = correctedComparisons.filter(
      (c) => c.significantAfterCorrection
    );
    ...
    return {
      ...
      overallPValue: Math.min(
        ...correctedComparisons.map((c) => c.correctedPValue)
      ),
    };
  }
  ...
  recommendWinner(experiment, variants, analysis) {
    const significantComparisons = analysis.correctedComparisons.filter(
      (c) => c.significantAfterCorrection
    );
    ...
    const winnerVariant = variants.find(
      (v) => v.id === bestComparison.treatment.id
    );

    return {
      winner: winnerVariant,
      confidence:
        bestComparison.effect.size === "large"
          ? "high"
          : bestComparison.effect.size === "medium"
          ? "medium"
          : "low",
      reason: `${winnerVariant.name} shows ${(
        bestComparison.effect.relative * 100
      ).toFixed(1)}% improvement with ${
        bestComparison.effect.size
      } effect size`,
  ...
  generateRecommendations(experiment, analysis) {
    ...
    if (power < this.statisticalConfig.powerAnalysisTarget) {
      recommendations.push({
        ...
        priority: "medium",
        message: `Current statistical power is ${(power * 100).toFixed(
          1
  ...
  async saveAnalysis(analysis) {
    const filePath = path.join(
      this.options.storageDir,
      "results",
      `${analysis.experimentId}_${analysis.timestamp}.json`
    );
