...
class HubSpokeMetrics extends MetricsCollector {
  ...
  async recordRoutingCompletion(data) {
    ...
    const requestEvent = this.routingRequests.find(
      (r) => r.requestId === data.requestId
    );
    if (requestEvent) {
      completionEvent.totalTime =
        completionEvent.endTime - requestEvent.startTime;
      completionEvent.routingTime =
        completionEvent.totalTime - completionEvent.agentResponseTime;
      completionEvent.overheadRatio =
        completionEvent.coordinationOverhead / completionEvent.totalTime;
  ...
  async recordViolation(data) {
    ...
    await this.store("compliance_violation", violationEvent, {
      hypothesis: this.hypothesis,
      alert:
        violationEvent.severity === "high" ||
        violationEvent.severity === "critical",
    });
  ...
  recordGraphEdge(pattern, type) {
    if (!this.agentCommunicationGraph.has(pattern)) {
      this.agentCommunicationGraph.set(pattern, {
        count: 0,
        type,
        timestamps: [],
      });
  ...
  performAggregation(metrics) {
    ...
    const routingMetrics = metrics.filter(
      (m) => m.eventType === "routing_request"
    );
    const completionMetrics = metrics.filter(
      (m) => m.eventType === "routing_completion"
    );
    const violationMetrics = metrics.filter(
      (m) => m.eventType === "compliance_violation"
    );
    const coordinationMetrics = metrics.filter(
      (m) => m.eventType === "coordination_event"
    );
    ...
    return {
      ...
      routing: {
        ...
        successfulRequests: completionMetrics.filter((m) => m.data.success)
          .length,
        hubMediatedRequests: routingMetrics.filter((m) => m.data.throughHub)
          .length,
        directCommunications: routingMetrics.filter((m) => !m.data.throughHub)
          .length,
        averageResponseTime:
          this.calculateAverageResponseTime(completionMetrics),
        routingCompliance: this.calculateRoutingCompliance(routingMetrics),
        coordinationOverhead:
          this.calculateAverageCoordinationOverhead(completionMetrics),
      },
      violations: {
      ...
      coordination: {
        ...
        coordinationEventsByType:
          this.countCoordinationEventsByType(coordinationMetrics),
        averageProcessingTime:
          this.calculateAverageProcessingTime(coordinationMetrics),
  ...
  calculateRoutingCompliance(routingMetrics) {
    ...
    const hubMediatedCount = routingMetrics.filter(
      (m) => m.data.throughHub
    ).length;
  ...
  calculateAverageResponseTime(completionMetrics) {
    ...
    const successfulMetrics = completionMetrics.filter(
      (m) => m && m.data && m.data.success && m.data.totalTime
    );
    if (successfulMetrics.length === 0) return 0;

    const totalTime = successfulMetrics.reduce(
      (sum, m) => sum + m.data.totalTime,
      0
    );
  ...
  calculateAverageCoordinationOverhead(completionMetrics) {
    ...
    const metricsWithOverhead = completionMetrics.filter(
      (m) => m && m.data && m.data.overheadRatio != null
    );
    if (metricsWithOverhead.length === 0) return 0;

    const totalOverhead = metricsWithOverhead.reduce(
      (sum, m) => sum + m.data.overheadRatio,
      0
    );
  ...
  calculateAutoResolutionRate(violationMetrics) {
    ...
    const autoResolvedCount = violationMetrics.filter(
      (m) => m.data.wasAutoResolved
    ).length;
  ...
  calculateAverageProcessingTime(coordinationMetrics) {
    const metricsWithTime = coordinationMetrics.filter(
      (m) => m.data.processingTime > 0
    );
    if (metricsWithTime.length === 0) return 0;

    const totalTime = metricsWithTime.reduce(
      (sum, m) => sum + m.data.processingTime,
      0
    );
  ...
  calculateHubUtilization(coordinationMetrics) {
    ...
    const timeSpan =
      coordinationMetrics.length > 1
        ? coordinationMetrics[coordinationMetrics.length - 1].timestamp -
          coordinationMetrics[0].timestamp
        : 0;
  ...
  analyzeCommunicationGraph() {
    const hubMediatedEdges = Array.from(
      this.agentCommunicationGraph.entries()
    ).filter(([_, data]) => data.type === "hub_mediated").length;

    const directEdges = Array.from(
      this.agentCommunicationGraph.entries()
    ).filter(([_, data]) => data.type === "direct").length;
  ...
  calculateCentralityMetrics() {
    ...
    const sortedAgents = Array.from(agentConnections.entries()).sort(
      (a, b) => b[1] - a[1]
    );
  ...
  analyzeLoadDistribution() {
    ...
    const variance =
      loads.reduce((sum, load) => sum + Math.pow(load - average, 2), 0) /
      loads.length;
    ...
    return {
      ...
      hypothesisValidation: {
        ...
        performanceImprovement:
          this.calculatePerformanceImprovement(completionMetrics),
      ...
      predictions: {
        ...
        timeToTargetCompliance:
          this.estimateTimeToTargetCompliance(routingMetrics),
  ...
  calculatePerformanceImprovement(completionMetrics) {
    ...
    const baseline =
      this.baseline?.measurements?.coordination?.coordinationOverhead || 1.0;
    const current =
      this.calculateAverageCoordinationOverhead(completionMetrics);
  ...
  calculateErrorReduction(completionMetrics) {
    ...
    const successRate =
      completionMetrics.filter((m) => m.data.success).length /
      completionMetrics.length;
    const baseline =
      this.baseline?.measurements?.coordination?.routingCompliance || 0;
  ...
  calculateViolationTrend(violationMetrics) {
    ...
    const recentViolations = violationMetrics.filter(
      (m) => now - m.timestamp < timeWindow
    ).length;

    const previousViolations = violationMetrics.filter(
      (m) =>
  ...
  projectScalability() {
    const currentAgents = Array.from(
      new Set(this.routingRequests.map((r) => r.toAgent))
    ).length;
  ...
  validateHypotheses(metrics) {
    ...
    const h2Validation = {
      hypothesis:
        "Hub-and-Spoke coordination outperforms distributed communication",
      validated: false,
      confidence: this.calculateConfidence(routingMetrics.length),
      evidence: [],
      metrics: {
        routingCompliance:
          analysis.hypothesisValidation?.routingCompliance || 0,
        coordinationOverhead:
          analysis.hypothesisValidation?.coordinationOverhead || 1,
        violationRate: analysis.hypothesisValidation?.violationRate || 1,
        performanceImprovement:
          analysis.hypothesisValidation?.performanceImprovement || 0,
      ...
      criteria: {
        ...
        confidenceThreshold:
          this.config.hypotheses.h2_hubSpoke.confidenceThreshold,
      },
    };

    // Check validation criteria
    const meetsCompliance =
      h2Validation.metrics.routingCompliance >=
      h2Validation.criteria.targetCompliance;
    const meetsOverhead =
      h2Validation.metrics.coordinationOverhead <=
      h2Validation.criteria.maxOverhead;
    const meetsConfidence =
      h2Validation.confidence >= h2Validation.criteria.confidenceThreshold;

    h2Validation.validated =
      meetsCompliance && meetsOverhead && meetsConfidence;

    // Collect evidence
    if (meetsCompliance) {
      h2Validation.evidence.push(
        `Routing compliance achieved ${(
          h2Validation.metrics.routingCompliance * 100
        ).toFixed(1)}% (target: ${(
          h2Validation.criteria.targetCompliance * 100
        ).toFixed(1)}%)`
      );
    }
    if (meetsOverhead) {
      h2Validation.evidence.push(
        `Coordination overhead maintained at ${(
          h2Validation.metrics.coordinationOverhead * 100
        ).toFixed(1)}% (target: <${(
          h2Validation.criteria.maxOverhead * 100
        ).toFixed(1)}%)`
      );
    }
    if (h2Validation.metrics.performanceImprovement > 0) {
      h2Validation.evidence.push(
        `Performance improved by ${(
          h2Validation.metrics.performanceImprovement * 100
        ).toFixed(1)}% over baseline`
      );
    }
    if (h2Validation.metrics.errorReduction > 0) {
      h2Validation.evidence.push(
        `Error rate reduced by ${(
          h2Validation.metrics.errorReduction * 100
        ).toFixed(1)}%`
      );
    }
