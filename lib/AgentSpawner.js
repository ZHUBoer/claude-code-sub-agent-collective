/**
 * claude tdd agents - Agent Spawner
 *
...
class AgentSpawner {
  constructor(options = {}) {
    ...
    this.agentsDir =
      options.agentsDir || path.join(process.cwd(), ".claude", "agents");
    this.testsDir =
      options.testsDir ||
      path.join(process.cwd(), ".claude-collective", "tests", "agents");
    this.archiveDir =
      options.archiveDir ||
      path.join(process.cwd(), ".claude-collective", "archive", "agents");
  ...
  async spawn(config) {
    ...
    try {
      ...
      if (this.activeSpawns.size >= this.config.maxConcurrentSpawns) {
        throw new Error(
          `Maximum concurrent spawns (${this.config.maxConcurrentSpawns}) exceeded`
        );
      }
      ...
      const agent = await this.templateSystem.createAgent(
        templateId,
        spawnParameters
      );
  ...
  async selectTemplate(config) {
    ...
    for (const template of templates) {
      ...
      if (
        purpose.includes("implement") &&
        template.id.includes("implementation")
      ) {
        score += 15;
      }
      ...
      if (config.tools) {
        ...
        const toolMatches = config.tools.filter(
          (tool) =>
      ...
      if (config.capabilities) {
        const templateDetails = this.templateSystem.getTemplate(template.id);
        const capabilityMatches = config.capabilities.filter(
          (cap) =>
            templateDetails.capabilities &&
            templateDetails.capabilities.includes(cap)
        ).length;
    ...
    const sortedTemplates = Array.from(scores.entries()).sort(
      (a, b) => b[1] - a[1]
    );
  ...
  prepareSpawnParameters(config, templateId) {
    return {
      ...
      specialization:
        config.specialization || this.inferSpecialization(config, templateId),

      // Arrays
      responsibilities:
        config.responsibilities || this.generateDefaultResponsibilities(config),
      tools: config.tools || this.inferRequiredTools(config, templateId),
      capabilities:
        config.capabilities || this.inferCapabilities(config, templateId),
      ...
      incomingHandoffs: config.incomingHandoffs || [
        {
          from: "@routing-agent",
          condition: "Task assignment and context transfer",
        },
      ],
      outgoingHandoffs: config.outgoingHandoffs || [
        {
          to: "@routing-agent",
          condition: "Task completion or escalation required",
        },
      ],
  ...
  inferSpecialization(config, templateId) {
    ...
    if (
      ...
      purpose.includes("validate") ||
      purpose.includes("quality")
    ) {
  ...
  inferRequiredTools(config, templateId) {
    ...
    if (
      purpose.includes("execute") ||
      purpose.includes("run") ||
      purpose.includes("command")
    ) {
  ...
  async validateSpawnParameters(templateId, parameters) {
    ...
    if (parameters.tools) {
      ...
      const invalidTools = parameters.tools.filter(
    ...
    if (parameters.agentName) {
      const existingAgentPath = path.join(
        this.agentsDir,
        `${parameters.agentName}.md`
      );
      if (await fs.pathExists(existingAgentPath)) {
        throw new Error(
          `Agent with name '${parameters.agentName}' already exists`
        );
      }
    }
  }
  ...
  getStatistics() {
    ...
    const avgDuration =
      this.spawnHistory.length > 0
        ? this.spawnHistory.reduce((sum, s) => sum + s.duration, 0) /
          this.spawnHistory.length
        : 0;

    const templateUsage = {};
    this.spawnHistory.forEach((spawn) => {
      if (spawn.templateId) {
        templateUsage[spawn.templateId] =
          (templateUsage[spawn.templateId] || 0) + 1;
      }
    });
