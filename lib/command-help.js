class CommandHelpSystem {
    ...
    this.addHelp("collective", {
      ...
      commands: {
        ...
        agents: {
          ...
          examples: ["/collective agents", "/collective agents --detailed"],
          flags: {
            "--detailed":
              "Show detailed agent information including status and capabilities",
          },
        },
        metrics: {
          syntax: "/collective metrics [--detailed]",
          description: "Display collective performance metrics",
          examples: ["/collective metrics", "/collective metrics --detailed"],
          flags: {
        ...
        validate: {
          ...
          arguments: {
            phase:
              "Optional: system, agents, hooks, behavioral (default: current)",
          },
          flags: {
        ...
        research: {
          ...
          arguments: {
            hypothesis:
              "h1_jitLoading (JIT Context Loading), h2_hubSpoke (Hub-Spoke Coordination), h3_tddHandoff (Test-Driven Handoffs)",
          },
        },
        coordinate: {
        ...
        history: {
          ...
          examples: ["/collective history", "/collective history 20"],
          arguments: {
    ...
    this.addHelp("agent", {
      ...
      commands: {
        spawn: {
          ...
          flags: {
            "--template":
              "Use specific agent template (default, custom, minimal)",
        ...
        list: {
          ...
          flags: {
            "--detailed":
              "Show detailed agent information including capabilities and status",
          },
        },
        status: {
        ...
        health: {
          ...
          examples: ["/agent health", "/agent health routing-agent --verbose"],
          arguments: {
            id: "Optional: Specific agent ID to check (default: all agents)",
          },
          flags: {
            "--verbose":
              "Show detailed health metrics including memory and performance",
          },
        },
        handoff: {
        ...
        help: {
          ...
          examples: ["/agent help", "/agent help spawn", "/agent help handoff"],
          arguments: {
    ...
    this.addHelp("gate", {
      ...
      commands: {
        status: {
          ...
          flags: {
            "--verbose":
              "Show detailed gate information including reasons for failures",
          },
        },
        validate: {
          ...
          arguments: {
            phase:
              "Optional: planning, infrastructure, implementation, testing, polish, completion (default: current)",
          },
          flags: {
            "--strict":
              "Use strict validation mode with enhanced quality checks",
          },
        },
        bypass: {
          ...
          arguments: {
            ...
            reason:
              "Detailed justification for bypass (required for audit trail)",
          },
          flags: {
            "--emergency":
              "Mark as emergency bypass for high-priority tracking",
          },
        },
        history: {
          syntax: "/gate history [limit]",
          description: "Show quality gate validation history",
          examples: ["/gate history", "/gate history 20"],
          arguments: {
        ...
        enforce: {
          ...
          arguments: {
            phase:
              "Phase to enforce: planning, infrastructure, implementation, testing, polish, completion",
          },
          flags: {
        ...
        report: {
          ...
          flags: {
            ...
            "--format":
              "Output format: json, markdown, html, csv (default: json)",
          },
        },
        help: {
          syntax: "/gate help [topic]",
          description: "Show gate command help",
          examples: ["/gate help", "/gate help bypass", "/gate help validate"],
          arguments: {
  ...
  getGeneralHelp() {
    let help = "# 🎯 claude tdd agents - Command System Help\n\n";
    help +=
      "The collective command system provides natural language and structured command interfaces for controlling the multi-agent orchestration system.\n\n";
    ...
    help +=
      '- "spawn a testing agent for integration" → `/agent spawn testing integration`\n\n';
    ...
    help +=
      "- **Command History**: Access recent commands with `/collective history`\n";
    help +=
      "- **Autocomplete**: Tab completion for commands, arguments, and flags\n";
    help += "- **Fuzzy Matching**: Typo-tolerant command suggestions\n";
    help += "- **Context Awareness**: Commands adapt based on system state\n";
    help +=
      "- **Performance Metrics**: All commands include execution timing\n\n";
  ...
  getNamespaceHelp(namespace) {
    ...
    if (!helpData) {
      const available = Array.from(this.helpTopics.keys()).filter(
        (k) => k !== "global"
      );
    ...
    if (namespace === "collective") {
      ...
      help +=
        "- Route complex tasks with `/collective route` for optimal agent selection\n";
      help += "- Check `/collective metrics` to validate research hypotheses\n";
    } else if (namespace === "agent") {
      help +=
        "- Use `/agent list --detailed` to understand available capabilities\n";
      help +=
        "- Spawn specialized agents with `/agent spawn [type] [specialization]`\n";
      help +=
        "- Monitor agent health with `/agent health` for optimal performance\n";
      ...
      "collective:route": [
        {
          ...
          solution:
            "Check available agents with `/agent list` and ensure your request matches agent capabilities",
        },
        {
          issue: "Routing takes too long",
          solution:
            "Use `--skip-test` flag to bypass validation tests for faster routing",
        },
      ],
      "collective:status": [
        {
          ...
          solution:
            "Run `/collective maintain --repair` to attempt automatic fixes, or check individual components",
        },
      ],
      "agent:spawn": [
        {
          ...
          solution:
            "Verify agent type is correct and check system resources with `/collective status`",
        },
        {
          issue: "Template not found",
          solution:
            "Use default template or check available templates in agent configuration",
        },
      ],
      "gate:bypass": [
        {
          ...
          solution:
            "Ensure you have proper permissions and provide a detailed reason for the bypass",
        },
        {
          issue: "Audit trail required",
          solution:
            "All bypasses are logged. Use `--emergency` flag only for critical situations",
        },
      ],
    };
  ...
  getContextualHelp(helpIntent, context) {
    ...
    if (context.systemState?.newUser) {
      ...
      help +=
        '- `/collective route "your task here"` - Route tasks to agents\n\n';
    }
