...
class VanMaintenanceSystem extends EventEmitter {
  constructor() {
    ...
    this.collectiveDir = path.join(this.projectDir, "claude-tdd-agents");
  ...
  async checkFileSystem() {
    ...
    const requiredPaths = [
      "claude-tdd-agents",
      "claude-tdd-agents/tests",
      "claude-tdd-agents/lib/metrics",
      ".claude/agents",
      ".claude/hooks",
      ".claude/docs",
      "CLAUDE.md",
    ...
    // Check permissions
    const executablePaths = [".claude/hooks"];
  ...
  async checkPerformance() {
    ...
    try {
      ...
      await execAsync("npm test -- --listTests --passWithNoTests", {
        cwd: this.collectiveDir,
        timeout: 5000,
      });
      ...
      for (const [type, items] of Object.entries(issuesByType)) {
        ...
        if (repairKey && this.repairs.has(repairKey)) {
          ...
          try {
            ...
            console.log(
              chalk.green(`  ✅ ${repair.name}: Fixed ${result.fixed} issues`)
            );
          } catch (error) {
            repairs.push({
              type: repairKey,
      ...
      if (issue.type === "mermaid-syntax-error") {
        ...
        if (mermaidMatch) {
          ...
          mermaidContent = mermaidContent.replace(
            /(\w+)\{[^}]*"[^}]*\}/g,
            (match, nodeName) => {
              return `${nodeName}{ DETERMINE ${nodeName
                .replace(/_/g, " ")
                .toUpperCase()} }`;
            }
          );
          ...
          content = content.replace(
            mermaidMatch[0],
            `${mermaidMatch[1]}${mermaidContent}${mermaidMatch[3]}`
          );
    ...
    if (await fs.pathExists(metricsDir)) {
      ...
      const oldMetrics = files.filter((f) => {
        ...
        if (date) {
          ...
          const daysOld =
            (Date.now() - fileDate.getTime()) / (24 * 60 * 60 * 1000);
          return daysOld > 30;
        }
        return false;
      });
  ...
  generateSummary(report) {
    ...
    lines.push(
      `Repairs Made: ${report.repairs.filter((r) => r.success).length}`
    );
    lines.push(
      `Optimizations: ${report.optimizations.filter((o) => o.success).length}`
    );
  ...
  async getBehavioralTemplate() {
    return `# claude tdd agents Controller
