...
class MergeStrategies {
  ...
  async areFilesIdentical(file1Path, file2Path) {
    try {
      ...
      if (
        !(await fs.pathExists(file1Path)) ||
        !(await fs.pathExists(file2Path))
      ) {
        return false;
      }
      ...
      console.warn(
        chalk.gray(
          `Warning: Could not compare files ${file1Path} and ${file2Path}: ${error.message}`
        )
      );
      return false;
    }
  ...
  async analyzeExistingSetup(templateDir = null) {
    ...
    if (await fs.pathExists(settingsPath)) {
      ...
      if (templateDir) {
        ...
        isIdentical = await this.areFilesIdentical(
          settingsPath,
    ...
    if (await fs.pathExists(hooksDir)) {
      ...
      const existingOurHooks = hookFiles.filter((file) =>
        ourHooks.includes(file)
      );
      ...
      if (templateDir && existingOurHooks.length > 0) {
        ...
        for (const hookFile of existingOurHooks) {
          ...
          const isIdentical = await this.areFilesIdentical(
            existingHookPath,
            templateHookPath
          );
          if (!isIdentical) {
    ...
    if (analysis.hasConflicts) {
      ...
      analysis.recommendations.push(
        "Existing files will be backed up and replaced"
      );
    } else {
      analysis.recommendations.push(
        "Clean installation - no conflicts detected"
      );
    }
      ...
      if (existing.hooks && ourSettings.hooks) {
        for (const [eventType, ourHooks] of Object.entries(ourSettings.hooks)) {
          if (existing.hooks[eventType]) {
            ...
            const ourMatchers = new Set(ourHooks.map((h) => h.matcher));

            const matcherOverlap = [...existingMatchers].filter((m) =>
              ourMatchers.has(m)
            );
...
# Restore claude-tdd-agents backup from ${new Date().toISOString()}

echo "🔄 Restoring claude-tdd-agents backup..."
...
`;
  ...
  getOurSettingsTemplate() {
    return {
      deniedTools: ["mcp__task-master-ai__initialize_project"],
      hooks: {
        SessionStart: [
          {
            matcher: "startup",
            hooks: [
              {
                type: "command",
                command:
                  "$CLAUDE_PROJECT_DIR/.claude/hooks/load-behavioral-system.sh",
              },
              {
                type: "command",
                command: "$CLAUDE_PROJECT_DIR/.claude/hooks/auto-branch.sh",
              },
            ],
          },
          {
            ...
            hooks: [
              {
                type: "command",
                command:
                  "$CLAUDE_PROJECT_DIR/.claude/hooks/block-destructive-commands.sh",
              },
            ],
          },
          {
            ...
            hooks: [
              {
                type: "command",
                command:
                  "$CLAUDE_PROJECT_DIR/.claude/hooks/directive-enforcer.sh",
              },
              {
                type: "command",
                command:
                  "$CLAUDE_PROJECT_DIR/.claude/hooks/collective-metrics.sh",
              },
              {
                type: "command",
            ...
            hooks: [
              {
                type: "command",
                command:
                  "$CLAUDE_PROJECT_DIR/.claude/hooks/test-driven-handoff.sh",
              },
              {
                type: "command",
                command:
              ...
              {
                type: "command",
                command: "$CLAUDE_PROJECT_DIR/.claude/hooks/auto-squash.sh",
              },
            ],
          },
          {
            ...
            hooks: [
              {
                type: "command",
                command:
                  "$CLAUDE_PROJECT_DIR/.claude/hooks/mock-deliverable-generator.sh",
              },
              {
                type: "command",
                command:
