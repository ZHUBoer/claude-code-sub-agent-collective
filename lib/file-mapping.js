...
class FileMapping {
  ...
  getScriptsMapping() {
    const scripts = [
      { file: "collective-mode.sh", description: "Toggle collective mode enable/disable/status" },
      { file: "van-maintenance-test.sh", description: "Simulate van maintenance scenarios" },
    ];

    return scripts.map((script) => ({
      source: path.join("scripts", script.file),
      target: path.join(this.paths.claude, "scripts", script.file),
      type: "script",
      required: false,
      executable: true,
      overwrite: true,
      description: `脚本: ${script.description}`,
    }));
  }
