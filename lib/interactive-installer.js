...
class InteractiveInstaller {
  ...
  displayAnalysis(analysis) {
    if (!analysis.hasConflicts) {
      ...
      console.log(chalk.gray(`  • ${analysis.existingFiles.length} 个现有文件`));
  ...
  async smartMergeFlow(analysis) {
    ...
    console.log(chalk.gray(`发现 ${analysis.conflicts.length} 个文件将被备份并覆盖：`));
    
    for (let i = 0; i < analysis.conflicts.length; i++) {
      const conflict = analysis.conflicts[i];
      console.log(chalk.gray(`• ${conflict.type}：${conflict.message || '将被备份并覆盖'}`));
    }
  ...
  async getConflictStrategy(conflict) {
    ...
    const { strategy } = await inquirer.prompt([{
      type: 'list',
      name: 'strategy',
      message: `如何处理 ${conflict.type === 'settings' ? 'settings.json' : '钩子文件 (hooks)'}？`,
      choices
    }]);
  ...
  async forceOverwriteFlow(analysis) {
    ...
    if (analysis.hasConflicts) {
      console.log(chalk.gray(`• ${analysis.conflicts.length} 个冲突文件将被覆盖`));
      console.log(chalk.gray(`• ${analysis.existingFiles.length} 个现有文件将受影响`));
    }
    ...
    if (analysis.hasConflicts) {
      ...
      for (const conflict of analysis.conflicts) {
        ...
        if (conflict.type === 'hooks') {
          console.log(chalk.gray(`  • ${conflict.conflictingFiles.length} 个钩子文件`));
        }
      }
    } else {
  ...
  async executeSkipConflicts(analysis) {
    ...
    try {
      ...
      console.log(chalk.green('\n✅ 安装完成，并已跳过冲突！'));
      console.log(chalk.yellow('⚠️  由于跳过部分文件，某些智能体集合功能可能无法使用'));
      console.log(chalk.blue('💡 建议使用“智能合并”模式重新运行，以体验完整功能'));
  ...
  async showDetailedAnalysis(analysis) {
    ...
    if (analysis.conflicts.length > 0) {
      ...
      for (const conflict of analysis.conflicts) {
        console.log(chalk.gray(`  • ${conflict.type}: ${conflict.message || '配置存在重叠'}`));
