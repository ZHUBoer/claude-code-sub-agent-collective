/**
 * claude tdd agents - Agent Spawn Command
 *
...
class AgentSpawnCommand {
  ...
  parseArguments(args) {
    ...
    switch (command) {
      case "quick":
        ...
        result.purpose =
          positional.slice(1).join(" ") || "General purpose processing";
        break;
  ...
  parseFilters(options) {
    ...
    if (options.capability)
      filters.capabilities = options.capability.split(",");
  ...
  async templateSpawn(args, context) {
    if (!args.templateId) {
      throw new Error(
        "Template ID is required. Use --template=<id> or specify as first argument."
      );
    }
    ...
    const spawnConfig = {
      ...
      purpose:
        args.options.purpose || `Specialized ${args.templateId} operations`,
      specialization: args.options.specialization,
      tools: args.options.tools ? args.options.tools.split(",") : undefined,
      responsibilities: args.options.responsibilities
        ? args.options.responsibilities.split(",")
        : undefined,
    };
