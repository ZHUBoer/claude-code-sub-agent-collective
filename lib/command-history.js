...
class CommandHistoryManager {
  ...
  async addCommand(command, result, executionTime = 0) {
    ...
    // Save history after adding a command
  ...
  getHistory(limit = 10, filter = {}) {
    ...
    if (filter.namespace) {
      filtered = filtered.filter(
        (e) => e.result.namespace === filter.namespace
      );
    }
    ...
    if (filter.command) {
      ...
      filtered = filtered.filter(
        (e) =>
          ...
          (e.result.command &&
            e.result.command.toLowerCase().includes(commandQuery))
      );
    }
  ...
  searchHistory(query, limit = 10) {
    ...
    const results = this.history.filter(
      (entry) =>
        entry.command.toLowerCase().includes(queryLower) ||
        (entry.result.output &&
          entry.result.output.toLowerCase().includes(queryLower)) ||
        (entry.result.error &&
          entry.result.error.toLowerCase().includes(queryLower))
    );
  ...
  getRecentCommands(limit = 5) {
    return this.history.slice(0, limit).map((entry) => entry.command);
  }
  ...
  getStatistics() {
    ...
    stats.naturalLanguageUsageRate =
      stats.total > 0 ? stats.naturalLanguageUsage / stats.total : 0;
  ...
  getFailureAnalysis() {
    ...
    failures.forEach((entry) => {
      ...
      analysis.byErrorType[errorType] =
        (analysis.byErrorType[errorType] || 0) + 1;
    });
  ...
  async clearOldHistory(daysOld = 30) {
    ...
    this.history = this.history.filter(
      (entry) => new Date(entry.metadata.timestamp) > cutoffDate
    );
  ...
  exportHistory(format = "json", filter = {}) {
    ...
    if (format === "json") {
      ...
      const headers =
        "Timestamp,Command,Success,Namespace,Command Type,Error,Execution Time,Session\n";
      const rows = historyToExport
        .map(
          (e) =>
            `"${e.metadata.timestamp}","${e.command}",${e.result.success},"${
              e.result.namespace || ""
            }","${e.result.command || ""}","${e.result.error || ""}",${
              e.metadata.executionTime || 0
            },"${e.metadata.session}"`
        )
        .join("\n");
      return headers + rows;
      ...
      md += "## Commands\n\n";
      md +=
        "| Timestamp | Command | Success | Namespace | Error | Execution Time |\n";
      md +=
        "|-----------|---------|---------|-----------|-------|----------------|\n";
      ...
      historyToExport.forEach((e) => {
        ...
        md += `| ${timestamp} | \`${e.command}\` | ${success} | ${
          e.result.namespace || "-"
        } | ${error} | ${execTime}ms |\n`;
      });
  ...
  getCommandSuggestions(partial, limit = 5) {
    ...
    this.history.forEach((entry) => {
      ...
      if (
        command.toLowerCase().startsWith(partial.toLowerCase()) ||
    ...
    for (let i = 0; i < this.history.length - 1; i++) {
      ...
      if (current.metadata.session === next.metadata.session) {
        ...
        if (timeDiff < 300000) {
          ...
          const existing = patterns.commandSequences.find(
