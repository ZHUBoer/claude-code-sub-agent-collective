...
class InstallationTester {
  ...
  async testBasicInstallation() {
    ...
    try {
      ...
      const { stdout, stderr } = await execAsync(
        'node ../claude-tdd-agentsinstall-collective.js init --force',
  ...
  async testMinimalInstallation() {
    ...
    try {
      ...
      const { stdout } = await execAsync(
        'node ../claude-tdd-agentsinstall-collective.js init --minimal --force',
        { cwd: testDir, timeout: 30000 }
      );
  ...
  async testValidationCommand() {
    ...
    try {
      ...
      // Run validation
      const { stdout } = await execAsync(
        'node ../claude-tdd-agentsinstall-collective.js validate',
  ...
  async testStatusCommand() {
    ...
    try {
      ...
      // Run status
      const { stdout } = await execAsync(
        'node ../claude-tdd-agentsinstall-collective.js status',
        { cwd: testDir, timeout: 15000 }
      );
  ...
  async testCleanCommand() {
    ...
    try {
      ...
      // Run clean
      await execAsync(
        'node ../claude-tdd-agentsinstall-collective.js clean --force',
        { cwd: testDir, timeout: 15000 }
      );
