...
class ClaudeCodeCollective {
  ...
  /**
   * @method install
   * @description 在指定路径安装 "claude-tdd-agents" 框架.
   * @param {string} [targetPath='.'] - 目标安装路径, 默认为当前目录.
   */
  async install(targetPath = '.') {
    console.log(`claude tdd agents v${this.version}`);
    console.log('正在安装 "claude-tdd-agents" 框架...');
  ...
  async validate(projectPath = '.') {
    console.log('正在校验 "claude-tdd-agents" 的安装...');
    ...
    try {
      ...
      if (failures.length === 0) {
        console.log('✅ "claude-tdd-agents" 校验通过!');
  ...
  /**
   * @method getInfo
   * @description 获取关于 "claude-tdd-agents" 框架的信息.
  ...
  getInfo() {
    return {
      ...
      description: '用于 Claude Code 的 "claude-tdd-agents" 框架, 具备 TDD 校验, hub-spoke 协调和自动化切换功能',
      features: [
        'TDD 校验框架',
        'Hub-Spoke Agent 协调', 
        '自动化 Agent 切换',
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    case 'info':
      ...
      break;
    default:
      console.log('用法: claude-tdd-agents <install|validate|info> [target-path]');
      console.log('  install   - 安装 "claude-tdd-agents" 框架');
