...
program
  ...
  .description(
    "Sub-agent collective framework for Claude Code with TDD validation and hub-spoke coordination"
  )
...
program
  ...
  .option(
    "--mode <mode>",
    "Installation mode: smart-merge, force, or skip-conflicts",
    "smart-merge"
  )
  .option(
    "--backup <strategy>",
    "Backup strategy: full, simple, or none",
    "full"
  )
  ...
  .action(async (path, options) => {
    try {
      ...
      if (options.yes) {
        ...
        console.log(
          chalk.gray("Using smart defaults for rapid deployment...\n")
        );
...
program
  ...
  .action(async (projectPath) => {
    try {
      ...
      console.log(
        `🧠 Behavioral System: ${
          status.behavioral ? "✅ Active" : "❌ Missing"
        }`
      );
      console.log(
        `🧪 Testing Framework: ${status.testing ? "✅ Ready" : "❌ Missing"}`
      );
...
${chalk.yellow("Installation Modes:")}
  ${chalk.gray(
    "Interactive:"
  )} Full menu-driven experience with conflict resolution
  ${chalk.gray(
    "Express:"
  )}     Automated installation using smart defaults (--yes)
  ${chalk.gray("Minimal:")}     Core components only (--minimal)

${chalk.yellow("Merge Strategies:")}
  ${chalk.gray(
    "smart-merge:"
  )}    Intelligently merge with existing configs (default)
