...
program
  ...
  .action(async (options) => {
    try {
      ...
      if (
        options.interactive ||
        (!options.minimal && !options.hooksOnly && !options.testingOnly)
      ) {
        const configurator = new CollectiveConfigurator();
        const interactiveConfig =
          await configurator.promptInstallationOptions();
        ...
        installConfig = {
          ...options,
          ...configurator.generateInstallationOptions(interactiveConfig),
        };
      }
      ...
      if (result.success) {
        console.log(
          chalk.green("\n🎉 claude tdd agents installed successfully!")
        );
        ...
        console.log("3. Test installation: npx claude-tdd-agents validate");
      ...
      console.log(chalk.bold("\n🛠️ Troubleshooting:"));
      console.log(
        "1. Check Node.js version: node --version (>=16.0.0 required)"
      );
      ...
      console.log(chalk.bold("\n📞 Getting Help:"));
      console.log("• Check status: npx claude-tdd-agents status");
      console.log("• Validate setup: npx claude-tdd-agents validate");
  ...
  .action(async () => {
    try {
      ...
      if (status.issues.length > 0) {
        ...
        console.log(
          chalk.blue('\n💡 Run "npx claude-tdd-agents init" to fix issues')
        );
  ...
  .action(async () => {
    try {
      ...
      if (passed === total) {
        ...
        console.log(
          chalk.yellow(
            '\n🔧 Run "npx claude-tdd-agents init --force" to repair'
          )
        );
        process.exit(1);
      }
...
program
  ...
  .action((options) => {
    ...
    console.log("\nFor now, you can:");
    console.log("• Update manually: npx claude-tdd-agents init --force");
    console.log("• Check current version: npx claude-tdd-agents --version");
    console.log("• Validate installation: npx claude-tdd-agents validate");
  });
  ...
  .action(async (options) => {
    try {
      ...
      if (failedTests.length === 0) {
        console.log(
          chalk.green("✅ No issues found - installation is healthy!")
        );
        return;
      }
      ...
      if (result.success) {
        ...
        console.log(
          chalk.blue("💡 Restart Claude Code to ensure changes take effect")
        );
      }
    } catch (error) {
      console.error(chalk.red("❌ Repair failed:"), error.message);
      console.log(
        "\nTry manual reinstallation: npx claude-tdd-agents init --force"
      );
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (options) => {
    try {
        ...
        const { confirm } = await require("inquirer").prompt([
          {
            ...
            name: "confirm",
            message:
              "Are you sure you want to remove the collective installation?",
            default: false,
          },
        ]);
      ...
      const filesToRemove = ["CLAUDE.md", ".claude", ".claude-collective"];
...
if (!process.argv.slice(2).length) {
  program.outputHelp();
  console.log(chalk.yellow("\n💡 Quick start: npx claude-tdd-agents init"));
}
