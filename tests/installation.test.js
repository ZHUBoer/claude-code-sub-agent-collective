...
describe("NPX Installation Tests", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "collective-install-test-")
    );
  ...
  describe("File Mapping System", () => {
    ...
    test("should handle different installation modes", () => {
      const standardMapping = new FileMapping(projectRoot, {
        mode: "standard",
      });
  ...
  describe("Installation Flow", () => {
    test("should perform express installation successfully", async () => {
      ...
      expect(await fs.pathExists(path.join(projectRoot, "CLAUDE.md"))).toBe(
        true
      );
    });
