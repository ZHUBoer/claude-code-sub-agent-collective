  ...
  return jest.fn().mockImplementation(() => ({
    ...
    getAgent: jest
      ...
      .mockReturnValue({
        ...
        template: "base",
        path: "/tmp/src",
        testPath: "/tmp/src.test",
        metadata: { tools: [], capabilities: [] },
        health: { status: "healthy", issues: [] },
      }),
    query: jest
      .fn()
      .mockReturnValue([
        { id: "a1", status: "active", health: { status: "healthy" } },
      ]),
...
describe("AgentSpawnCommand - unit", () => {
  ...
  test("template spawn requires template id", async () => {
    await expect(cmd.execute("template")).resolves.toMatchObject({
      success: false,
    });
  });

  test("clone agent requires sourceId", async () => {
    await expect(cmd.execute("clone")).resolves.toMatchObject({
      success: false,
    });
  });
  ...
  test("template spawn success", async () => {
    const res = await cmd.execute(
      "template base --name=myagent --purpose=work"
    );
    expect(res.success).toBe(true);
