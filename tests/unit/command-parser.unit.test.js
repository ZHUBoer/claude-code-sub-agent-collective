...
describe('CollectiveCommandParser - unit', () => {
  ...
  test('expandAliases should expand known aliases', () => {
    ...
    expect(parser.expandAliases('/route create component')).toBe('/collective route create component');
  ...
  test('gate bypass should strip surrounding quotes in reason', async () => {
    const result = await parser.gateBypass(['testing', '"Emergency deploy"'], {});
  ...
  test('collective handlers return structured outputs', async () => {
    ...
    const validate = await parser.collectiveValidate(['current'], { strict: true });
    ...
    const coord = await parser.collectiveCoordinate(['implement feature X'], {});
  ...
  test('agent handlers cover error and normal paths', async () => {
    ...
    const status = await parser.agentStatus(['routing-agent'], { verbose: true });
    expect(status.action).toBe('status');
    ...
    const health = await parser.agentHealth(['routing-agent'], { verbose: true });
    expect(health.action).toBe('health');

    const handoffErr = await parser.agentHandoff([], {});
    expect(handoffErr.error).toBeTruthy();

    const metrics = await parser.agentMetrics(['routing-agent'], { detailed: true });
    expect(metrics.action).toBe('metrics');
