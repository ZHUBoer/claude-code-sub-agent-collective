...
const makeRegistry = () => {
  ...
  return {
    ...
    query: jest.fn().mockReturnValue([
      { id: 'a1', name: 'n1', status: 'active',
        activity: { lastSeen: new Date(Date.now() - 2 * 3600_000).toISOString(), invocations: 20 },
        performance: { successRate: 0.5, avgResponseTime: 100 },
        resources: { memoryUsage: 999999999, cpuUsage: 90, diskUsage: 9999999 },
        metadata: { registeredAt: new Date(Date.now() - 2 * 86400_000).toISOString() }
      }
    ]),
    unregister: jest.fn().mockResolvedValue(undefined),
    checkHealth: jest.fn().mockResolvedValue({ agentId: 'a1', status: 'healthy', timestamp: new Date().toISOString(), issues: [] }),
    saveRegistry: jest.fn().mockResolvedValue(undefined)
  };
};

const makeSpawner = () => ({
  spawn: jest.fn().mockResolvedValue({ agent: { id: 'auto', name: 'auto-scaled-agent' } })
});
