...
describe('AgentTemplateSystem - unit', () => {
  ...
  test('validateParameters enforces requireds and types', () => {
    ...
    expect(() => sys.validateParameters(t, { agentName: 'a', purpose: 'p', responsibilities: 'x' })).toThrow(/array/);
    // Valid
    expect(() => sys.validateParameters(t, { agentName: 'a', purpose: 'p', responsibilities: [] })).not.toThrow();
  });

  test('createAgent compiles template content', async () => {
    const res = await sys.createAgent('base', { agentName: 'alpha', purpose: 'demo' });
