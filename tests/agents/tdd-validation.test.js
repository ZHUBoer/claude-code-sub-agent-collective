  ...
  describe("Agent Integration", () => {
    test("should have tdd-validation-agent specification file", () => {
      const agentPath = path.join(
        __dirname,
        "../../templates/agents/tdd-validation-agent.md"
      );
...
function validateTDDCompliance(tddData) {
  ...
  const missingPhases = requiredPhases.filter(
    (phase) => !presentPhases.includes(phase)
  );
...
function validateBuildGate(buildResults) {
  const canProgress =
    ...
    buildResults.typecheckPass &&
    buildResults.testsPassing;
  const warnings = [];
...
function validateTDDHandoff(handoff) {
  ...
  const tddCompliant =
    evidence.redPhase?.testsWritten && evidence.greenPhase?.testsPass;
  const evidenceComplete = !!(
    evidence.redPhase &&
    evidence.greenPhase &&
    evidence.refactorPhase
  );
...
function validateTaskCompletion(taskValidation) {
  ...
  const completedPhases = tddPhases.filter(
    (phase) => taskValidation.tddCycle[phase].status === "complete"
  );
