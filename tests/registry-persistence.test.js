...
describe("Registry Persistence and Recovery Tests", () => {
  ...
  describe("Registry Persistence", () => {
    ...
    test("should auto-save on registry changes", async () => {
      ...
      await registry.updateStatus("auto-save-agent", "processing", {
        reason: "test update",
      });
  ...
  describe("Registry Recovery", () => {
    test("should recover complete registry state after restart", async () => {
      ...
      await registry.updateActivity("recovery-agent-1", {
        processingTime: 1000,
        success: true,
      });
      await registry.updateActivity("recovery-agent-2", {
        processingTime: 500,
        success: false,
      });
      ...
      expect(recoveredStats.currentStats.totalAgents).toBe(
        originalStats.currentStats.totalAgents
      );
    ...
    test("should handle corrupted registry file", async () => {
      ...
      const corruptedBackup = backupFiles.find((f) =>
        f.startsWith("corrupted-")
      );
  ...
  describe("Orphaned Agent Detection", () => {
    ...
    test("should cleanup orphaned registry entries", async () => {
      ...
      expect(cleanupResult.agents).toEqual([
        "orphaned-agent-1",
        "orphaned-agent-2",
      ]);
      ...
      operations.push(
        new Promise((resolve) =>
          setTimeout(
            () =>
              resolve(
                registry.updateStatus("concurrent-agent-0", "processing")
              ),
            10
          )
        ),
        new Promise((resolve) =>
          setTimeout(
            () =>
              resolve(
                registry.updateActivity("concurrent-agent-1", {
                  processingTime: 100,
                })
              ),
            20
  ...
  describe("Data Integrity", () => {
    test("should maintain data consistency across operations", async () => {
      ...
      await registry.updateActivity("integrity-test-agent", {
        processingTime: 500,
        success: true,
      });
      await registry.updateActivity("integrity-test-agent", {
