...
5.  **增强每个任务** - 通过 `mcp__task-master-ai__update_task` 添加 `research_context` 字段。
...
# 对于每个任务和相关的已发现技术：
#   mcp__task-master-ai__update_task(id="X", projectRoot="/path", prompt="研究上下文：来自 @.taskmaster/docs/research/{technology}-patterns.md 的 {technology} 发现")
# 示例：mcp__task-master-ai__update_task(id="1", projectRoot="/path", prompt="研究上下文：来自 @.taskmaster/docs/research/nextjs-patterns.md 的 Next.js 发现")
...
mcp__task-master-ai__get_tasks(projectRoot="/path") # 必须显示 research_context 字段
...
```json
{
  ...
  "research_context": {
    ...
    "key_findings": [
      "Next.js 14 中间件在 Edge Runtime 上运行",
      "Supabase Auth 需要服务器组件",
      "会话管理需要 cookies"
    ]
  },
  "implementation_guidance": {
...
- **研究参考**：直接指向已发现技术的缓存研究文档的 @ 文件路径（例如，@.taskmaster/docs/research/2025-08-10\_{technology}-patterns.md）
...
为初始任务生成执行了 `mcp__task-master-ai__parse_prd`
自主分析：使用研究缓存加载了 ResearchDrivenAnalyzer
复杂性评分：根据 Context7 研究模式对每个任务进行了分析
选择性扩展：仅根据研究驱动的评分扩展了高复杂性任务（得分 >5）
为研究驱动的选择性扩展执行了 [Z] 次 `mcp__task-master-ai__expand_task`
为研究上下文增强执行了 [W] 次 `mcp__task-master-ai__update_task`
...
- `mcp__task-master-ai__parse_prd` - 从 PRD 生成任务
- `mcp__task-master-ai__expand_task` - 仅选择性地扩展高复杂性任务
- `mcp__task-master-ai__update_task` - 用 `research_context` 和 `implementation_guidance` 增强任务
- `mcp__task-master-ai__get_tasks` - 检索任务以供 ResearchDrivenAnalyzer 分析
- `mcp__task-master-ai__generate` - 增强后生成最终的任务文件
