...
1.  **分析任务队列** - 使用 `projectRoot` 执行 `mcp__task-master-ai__get_tasks`。
...
1.  **分析任务并创建部署计划**：
    ...
    - 使用 `mcp__task-master-ai__get_tasks` 检索所有可用任务。
...
3.  **监控协调的执行**：
    - 通过 `mcp__task-master-ai__get_task` 跟踪任务状态更新。
    - 等待代理的完成报告和可交付成果证据。
...
- 成功标准：[具体的完成要求]
- 上下文：[相关的项目信息]
- 报告：完成时使用 mcp__task-master-ai__set_task_status
```
...
```
任务队列已分析：[列出通过 mcp__task-master-ai__get_tasks 找到的实际任务]
...
```
`mcp__task-master-ai__get_tasks` 已使用 projectRoot 执行 [X] 次
`mcp__task-master-ai__get_task` 已为任务分析执行 [X] 次
`LS`/`Read` 工具已为可交付成果验证执行 [X] 次
`mcp__task-master-ai__set_task_status` 已为进度跟踪执行 [X] 次
```
